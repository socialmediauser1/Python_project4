class TemplateEngine:
    ...
    def render_vars(self, str: str, context: dict) -> str:
        index, out, length = 0, [], len(str)
        while index < length:
            open_pos = str.find("{{", index)
            if open_pos == -1:
                out.append(str[index:]); break
            out.append(str[index:open_pos])
            close_pos = str.find("}}", open_pos + 2)
            if close_pos == -1:
                out.append(str[open_pos:]); break
            key = str[open_pos+2:close_pos].strip()
            out.append("" if key == "" else str(context.get(key, "")))
            index = close_pos + 2
        return "".join(out)

    def render_if_once(self, str: str, context: dict) -> str:
        start = str.find("{% if ")
        if start == -1:
            return str
        head_end = str.find("%}", start)
        if head_end == -1:
            return str
        index, depth, block_end = head_end + 2, 1, -1
        elif_positions, else_position = [], -1
        while index < len(str):
            next_if = str.find("{% if ", index)
            next_elif = str.find("{% elif", index)
            next_else = str.find("{% else %}", index)
            next_end = str.find("{% endif %}", index)
        ...
        condition = str[start+len("{% if "):head_end].strip()
        headers = [("if", condition, start, head_end+2)]
        for position in elif_positions:
            tag_end = str.find("%}", position)
            headers.append(("elif", str[position+len("{% elif"):tag_end].strip(), position, tag_end+2))
        if else_position != -1:
        ...
        for index, header in enumerate(headers):
            ...
            content = str[header_end:next_start]
        ...
        after = str.find("{% endif %}", block_end) + len("{% endif %}")
        return str[:start] + inner + str[after:]

    def evaluate(self, expr: str, context: dict) -> bool:
        expr = expr.strip()
        operators = ["==", "!=", ">=", "<=", ">", "<"]
        for operator in operators:
            position = expr.find(operator)
            if position != -1:
                left = expr[:position].strip()
                right = expr[position+len(operator):].strip()
                ...
                try:
                    ...
                    return False
        return bool(context.get(expr, None))

    def to_num(self, str: str, context: dict):
        if str and (str.isdigit() or (str[0] == "-" and str[1:].isdigit())):
            try: return int(str)
            except Exception: pass
        if len(str) >= 2 and str[0] == str[-1] and str[0] in ("'", '"'):
            return str[1:-1]
        return context.get(str, "")

    def render_for_once(self, str: str, context: dict) -> str:
        start = str.find("{% for ")
        if start == -1:
            return str
        head_end = str.find("%}", start)
        ...
        index, depth, block_end = head_end + 2, 1, -1
        while index < len(str):
            next_for = str.find("{% for ", index)
            next_end = str.find("{% endfor %}", index)
        ...
        header = str[start+len("{% for "):head_end].strip()
        ...
        inner = str[head_end+2:block_end]
        ...
        after = str.find("{% endfor %}", block_end) + len("{% endfor %}")
        return str[:start] + "".join(rendered) + str[after:]